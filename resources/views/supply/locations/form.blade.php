...
@endphp
<x-supply-layout :title="(isset($location) ? 'Edit ' : 'Add ') . $label" subtitle="Location used for stock transfers and tracking.">
    <form method="POST" action="{{ isset($location) ? route($updateRoute, $location) : route($storeRoute) }}" class="bg-white rounded-2xl border p-6 space-y-4 max-w-2xl">
        ...
        <label class="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" name="is_default" value="1" @checked(old('is_default', $location->is_default ?? false))> Set as default {{ strtolower($label) }}</label>
        @if(isset($location))
